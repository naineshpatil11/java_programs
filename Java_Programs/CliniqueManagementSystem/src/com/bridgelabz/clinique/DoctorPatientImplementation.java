...
{
	 //implements DoctorAndPatientData 
		...
		List<Appoitment> appointmentlist = new ArrayList<>(); // create arraylist to store appointment details
		{
			try {
				// read files and store data into linked list
				personlist = mapper.readValue(new File(
						"/home/user/Documents/Java_Programs/CliniqueManagementSystem/Patients.json"),
						new TypeReference<List<Patients>>() {
						});
				doctorlist = mapper.readValue(new File(
						"/home/user/Documents/Java_Programs/CliniqueManagementSystem/Doctors.json"),
						new TypeReference<List<Doctor>>() {
						});
				appointmentlist = mapper.readValue(new File(
						"/home/user/Documents/Java_Programs/CliniqueManagementSystem/Appointment.json"),
						new TypeReference<List<Appoitment>>() {
						});
		...
		public void operation() {
			...
			do {System.out.println("______________________");
				...
				Answer = Utility.inputIntegerValue();
				switch (Answer) {
				case 1:
				// to add doctor
					addDoctor();
		...
		//	System.out.println("Enter Doctor id");
			id = Utility.inputInteger("Enter Doctor id");
			int i = 0;
	...
	//add new patient
		public void addPatient() {
			...
			age = Utility.inputInteger("Enter Patient Age");
			id = 0;
		...
		public void deletePatient() {
			...
			id = Utility.inputInteger("Enter Patient id");
			int i = 0;
	...
	//fix appointment
		public void fixAppointment() {
			...
			int doctorId = Utility.inputInteger("Enter DoctorId");
			System.out.println("Enter PatientId");
			int patientId = Utility.inputInteger("Enter PatientId");
			...
			appointmentlist
					.add(new Appoitment(objtemp.getName(), doctorId, objtemp2.getName(), patientId, patientphone, avail));
